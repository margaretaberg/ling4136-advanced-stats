...
pacman::p_load(rstudioapi, tidyverse, lme4, afex)
...
-   getting familiar with identifying random effects
...
In this dataset, we will be analyzing data from the following paper:

Winter & Grawunder. 2012. The phonetic profile of Korean formal and informal speech registers, Journal of Phonetics 40(6). 808-815. https://doi.org/10.1016/j.wocn.2012.08.006
...
-   Are there non-independent data points?

If so, what are the grouping variables? If you determine that there are non-independencies to be controlled for, you would need a mixed model.
...
# Since we want to predict the mean pitch for informal and formal contexts, we want to run a linear regression on the pitch column. Since we know participants' sex plays a large role too, we add it to the model
...
The final model does not converge (surprise!). Run `help('isSingular')` to learn more about this warning, but bottom line is:\
we should not trust its estimates. What to do? One strategy that is sometimes recommended and often implemented is reducing the random slope complexity.

We start by removing one random slope over time. As a rule of thumb, variability across subjects is often much higher than across items, so a good point of departure is removing the slope for `sentences`:
